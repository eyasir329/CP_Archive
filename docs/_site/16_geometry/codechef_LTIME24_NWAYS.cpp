#include <bits/stdc++.h>
using namespace std;

#define endl '\n'
using ll = long long;

const int N = 1e7 + 7, mod = 1e9 + 7;  // N must be less than mod

int fact[N], ifact[N];

int power(int x, int n) {  // O(logn)
  int ans = 1;
  while (n > 0) {
    if (n & 1) {
      ans = 1LL * ans * x % mod;
    }
    x = 1LL * x * x % mod;
    n >>= 1;
  }
  return ans;
}

int inverse(int a) {  // O(logmod)
  return power(a, mod - 2);
}

void prec() {  // O(n)
  fact[0] = 1;
  for (int i = 1; i < N; i++) {
    fact[i] = 1LL * fact[i - 1] * i % mod;
  }
  ifact[N - 1] = inverse(fact[N - 1]);
  for (int i = N - 2; i >= 0; i--) {
    ifact[i] = 1LL * ifact[i + 1] * (i + 1) % mod;  // 1/i! = (1/(i+1)!)*(i+1)
  }
}

int nCr(int n, int r) {  // O(1)
  if (n < r) return 0;
  return 1LL * fact[n] * ifact[r] % mod * ifact[n - r] % mod;
}

int32_t main() {
  ios_base::sync_with_stdio(false);
  cin.tie(NULL);
  prec();
  int t;
  cin >> t;
  while (t--) {
    int n, k;
    cin >> n >> k;
    set<pair<int, int>> a, b;
    for (int i = 1; i <= n; i++) {
      a.insert({0, i});
      b.insert({k, i});
    }
    // subtask1-> 1 ≤ T ≤ 20, 1 ≤ N ,K ≤ 1000
    // We have to reach to the point (c,d) from the point (a,b) using abs(a-c)+
    // abs(b-d) steps only.
    ll ans = 0;
    for (auto [x1, y1] : a) {
      for (auto [x2, y2] : b) {
        int dx = abs(x2 - x1);
        int dy = abs(y2 - y1);
        int totalSteps = dx + dy;
        ll cur =
            (((1LL * fact[totalSteps] * ifact[dx]) % mod) * ifact[dy]) % mod;
        ans = (1LL * ans + cur) % mod;
      }
    }
    // S = 2 * C(n + k + 1, k + 2) - n -> log1
    cout << ans << endl;
  }
  return 0;
}